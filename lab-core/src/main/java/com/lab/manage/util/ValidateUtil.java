...
public class ValidateUtil {
     ...
     */
    private static final String _PHONE_REGEX_PATTERN = "((^(13|14|15|16|17|18|19)[0-9]{9}$)|(^0[1,2]{1}\\d{1}-?\\d{8}$)|(^0[3-9]{1}\\d{2}-?\\d{7,8}$)|(^0[1,2]{1}\\d{1}-?\\d{8}-(\\d{1,4})$)|(^0[3-9]{1}\\d{2}-?\\d{7,8}-(\\d{1,4})$))";
    /**
     * 是否是null
     * @param term
     * @return
     ...
     * @return
     */
    public static boolean isNotNull(java.util.Iterator<?> iterator) {
     ...
     * @return
     */
    public static boolean isNotNullCollection(Collection<?> colles) {
        return !isNullCollection(colles);
    }
